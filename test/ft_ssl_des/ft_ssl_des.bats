...
@test "base64 0B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 0B -o ft_ssl_base64_0B_enc
...
@test "base64 1B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 1B -o ft_ssl_base64_1B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 2B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 2B -o ft_ssl_base64_2B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 3B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 3B -o ft_ssl_base64_3B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 4B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 4B -o ft_ssl_base64_4B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 5B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 5B -o ft_ssl_base64_5B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 15B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 15B -o ft_ssl_base64_15B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 16B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 16B -o ft_ssl_base64_16B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 17B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 17B -o ft_ssl_base64_17B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 32B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 32B -o ft_ssl_base64_32B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 64B" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 64B -o ft_ssl_base64_64B_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 1KB" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 1KB -o ft_ssl_base64_1KB_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 4KB" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 4KB -o ft_ssl_base64_4KB_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 64KB" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 64KB -o ft_ssl_base64_64KB_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 1MB" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 1MB -o ft_ssl_base64_1MB_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 10MB" {
    local valgrind_log=$(mktemp)
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -i 10MB -o ft_ssl_base64_10MB_enc
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 0B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 0B_enc -o ft_ssl_base64_0B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 1B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 1B_enc -o ft_ssl_base64_1B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 2B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 2B_enc -o ft_ssl_base64_2B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 3B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 3B_enc -o ft_ssl_base64_3B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 4B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 4B_enc -o ft_ssl_base64_4B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 5B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 5B_enc -o ft_ssl_base64_5B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 15B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 15B_enc -o ft_ssl_base64_15B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 16B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 16B_enc -o ft_ssl_base64_16B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 17B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 17B_enc -o ft_ssl_base64_17B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 32B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 32B_enc -o ft_ssl_base64_32B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 64B" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 64B_enc -o ft_ssl_base64_64B_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 1KB" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 1KB_enc -o ft_ssl_base64_1KB_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 4KB" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 4KB_enc -o ft_ssl_base64_4KB_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 64KB" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 64KB_enc -o ft_ssl_base64_64KB_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 1MB" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 1MB_enc -o ft_ssl_base64_1MB_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
...
@test "base64 -d 10MB" {
    ...
    valgrind --log-file="$valgrind_log" ../ft_ssl base64 -d -i 10MB_enc -o ft_ssl_base64_10MB_dec
    run grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log"
    assert_success
    run grep -q "ERROR SUMMARY: 0 errors from 0 contexts" "$valgrind_log"
    assert_success
